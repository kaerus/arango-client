...
return function(db) {
  ...
  function Query(){ 
    ...
    query.exec = function() { 
      ...
      on_next = function(res,hdr){
        db.cursor.get(res.id,callback).on('result',on_next).on('error',on_error);
        this.emit("next",res,hdr);  
      }, 
      ret = exec_cursor("create",arguments).on('result', on_next);
    };
