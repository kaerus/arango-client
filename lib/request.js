    ...
    function transport(options,data,callback) {
        ...
        var xhr = Ajax.request(options,function(res) {
            ...
            res.on('end',function(){
                ...
                if(response.error) {
                    error = {error: true, code: response.code, message: response.errorMessage};
                } else if(res.error) {
                    error = {error: true, code: res.statusCode, message: response.message};
                } 
                ...
                if(error) {
                    ...
                    promise.fullfilled(response, res.headers, res.statusCode)
                }
            });
        ...
        if(callback) {
            return promise.then(function(result,headers,code) {
                return callback(null,result,headers,code);
            },function(err) {
