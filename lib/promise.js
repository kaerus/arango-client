...
define(function (require) {
    ...
    Promise.prototype.include = function(includePromise) {
        var self = this, promise = new Promise();

        if(!this.calls) this.calls = [];

        function includeFulfill(left) {

            includePromise.then(function(right){
                promise.fulfill([left,right]);
            },function(reason){
                promise.reject(reason);
            });

            if(includePromise.resolved) {
                setImmediate(function(){
                    includePromise.resolve();
                });
            }
