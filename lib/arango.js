...
function Connection(){
  ...
  this.config = {
    server: { 
      protocol: options.protocol ? options.protocol + ':' : window ? window.location.protocol : "http:",
      hostname: options.hostname || window ? window.location.hostname : "127.0.0.1",
      port: parseInt(options.port,10) || window ? window.location.port : 8529
    },
    user: options.user || "",
    _pass: options.pass || "",
    name: options.name || ""  
